...
template< class Real >
class PlyValueOrientedVertex
{
public:
	typedef PlyValueOrientedVertex Wrapper;
	...
	PlyValueOrientedVertex(void) { ; }
	PlyValueOrientedVertex(Point3D< Real > p, Real v, Point3D< Real > n) : point(p), value(v), normal(n){ ; }
	PlyValueOrientedVertex operator + (PlyValueOrientedVertex p) const { return PlyValueOrientedVertex(point + p.point, normal + p.normal); }
	PlyValueOrientedVertex operator - (PlyValueOrientedVertex p) const { return PlyValueOrientedVertex(point - p.value, normal - p.normal); }
	template< class _Real > PlyValueOrientedVertex operator * (_Real s) const { return PlyValueOrientedVertex(point*s, normal*s); }
	template< class _Real > PlyValueOrientedVertex operator / (_Real s) const { return PlyValueOrientedVertex(point / s, normal / s); }
	PlyValueOrientedVertex& operator += (PlyValueOrientedVertex p) { point += p.point, normal += p.normal; return *this; }
	PlyValueOrientedVertex& operator -= (PlyValueOrientedVertex p) { point -= p.point, normal -= p.normal; return *this; }
	template< class _Real > PlyValueOrientedVertex& operator *= (_Real s) { point *= s, normal *= s; return *this; }
	template< class _Real > PlyValueOrientedVertex& operator /= (_Real s) { point /= s, normal /= s; return *this; }
};
template< class Real, class _Real > PlyValueOrientedVertex< Real > operator * (XForm4x4< _Real > xForm, PlyValueOrientedVertex< Real > v) { return PlyValueOrientedVertex< Real >(xForm * v.point, xForm.inverse().transpose() * v.normal); }
template< class Real > PlyProperty PlyValueOrientedVertex< Real >::ReadProperties[] =
{
	{ _strdup("x")		, PLYType< Real >() , PLYType< Real >() , int(offsetof(PlyValueOrientedVertex ,  point.coords[0])) , 0 , 0 , 0 , 0 },
	{ _strdup("y")		, PLYType< Real >() , PLYType< Real >() , int(offsetof(PlyValueOrientedVertex ,  point.coords[1])) , 0 , 0 , 0 , 0 },
	{ _strdup("z")		, PLYType< Real >() , PLYType< Real >() , int(offsetof(PlyValueOrientedVertex ,  point.coords[2])) , 0 , 0 , 0 , 0 },
	{ _strdup("value")  , PLYType< Real >() , PLYType< Real >() , int(offsetof(PlyValueOrientedVertex ,  value)) , 0 , 0 , 0 , 0 } ,
	{ _strdup("nx")		, PLYType< Real >() , PLYType< Real >() , int(offsetof(PlyValueOrientedVertex , normal.coords[0])) , 0 , 0 , 0 , 0 },
	{ _strdup("ny")		, PLYType< Real >() , PLYType< Real >() , int(offsetof(PlyValueOrientedVertex , normal.coords[1])) , 0 , 0 , 0 , 0 },
	{ _strdup("nz")		, PLYType< Real >() , PLYType< Real >() , int(offsetof(PlyValueOrientedVertex , normal.coords[2])) , 0 , 0 , 0 , 0 }
};
template< class Real > PlyProperty PlyValueOrientedVertex< Real >::WriteProperties[] =
{
	{ _strdup("x") , PLYType< Real >() , PLYType< Real >() , int(offsetof(PlyValueOrientedVertex ,  point.coords[0])) , 0 , 0 , 0 , 0 },
	{ _strdup("y") , PLYType< Real >() , PLYType< Real >() , int(offsetof(PlyValueOrientedVertex ,  point.coords[1])) , 0 , 0 , 0 , 0 },
	{ _strdup("z") , PLYType< Real >() , PLYType< Real >() , int(offsetof(PlyValueOrientedVertex ,  point.coords[2])) , 0 , 0 , 0 , 0 },
...
template< class Real >
class PlyOrientedColorVertex
{
public:
	struct _PlyOrientedColorVertex
	{
		Point3D< Real > point, normal, color;
		_PlyOrientedColorVertex(void) { ; }
		_PlyOrientedColorVertex(Point3D< Real > p, Point3D< Real > n, Point3D< Real > c) : point(p), normal(n), color(c) {; }
		_PlyOrientedColorVertex(PlyOrientedColorVertex< Real > p){ point = p.point; normal = p.normal; for (int c = 0; c < 3; c++) color[c] = (Real)p.color[c]; }
		operator PlyOrientedColorVertex< Real >()
		{
			PlyOrientedColorVertex< Real > p;
			p.point = point;
			p.normal = normal;
		...
		_PlyOrientedColorVertex operator + (_PlyOrientedColorVertex p) const { return _PlyOrientedColorVertex(point + p.point, normal + p.normal, color + p.color); }
		_PlyOrientedColorVertex operator - (_PlyOrientedColorVertex p) const { return _PlyOrientedColorVertex(point - p.point, normal - p.normal, color - p.color); }
		template< class _Real > _PlyOrientedColorVertex operator * (_Real s) const { return _PlyOrientedColorVertex(point*s, normal*s, color*s); }
		template< class _Real > _PlyOrientedColorVertex operator / (_Real s) const { return _PlyOrientedColorVertex(point / s, normal/s, color / s); }
		_PlyOrientedColorVertex& operator += (_PlyOrientedColorVertex p) { point += p.point, normal += p.normal, color += p.color; return *this; }
		_PlyOrientedColorVertex& operator -= (_PlyOrientedColorVertex p) { point -= p.point, normal -= p.normal, color -= p.color; return *this; }
		template< class _Real > _PlyOrientedColorVertex& operator *= (_Real s) { point *= s, normal *= s; color *= s; return *this; }
		template< class _Real > _PlyOrientedColorVertex& operator /= (_Real s) { point /= s, normal /= s; color /= s; return *this; }
	};

	typedef _PlyOrientedColorVertex Wrapper;
	...
	PlyOrientedColorVertex(void) { point.coords[0] = point.coords[1] = point.coords[2] = 0, normal.coords[0] = normal.coords[1] = normal.coords[2] = 0; color[0] = color[1] = color[2] = 0; }
	PlyOrientedColorVertex(const Point3D<Real>& p) { point = p; }
	PlyOrientedColorVertex(const Point3D< Real >& p, const unsigned char c[3]) { point = p, color[0] = c[0], color[1] = c[1], color[2] = c[2]; }
};
template< class Real, class _Real > PlyOrientedColorVertex< Real > operator * (XForm4x4< _Real > xForm, PlyOrientedColorVertex< Real > v) { return PlyOrientedColorVertex< Real >(xForm * v.point, xForm.inverse().transpose() * v.normal, v.color); }

template< class Real > PlyProperty PlyOrientedColorVertex< Real >::ReadProperties[] =
{
	{ _strdup("x"), PLYType< Real >(), PLYType< Real >(), int(offsetof(PlyOrientedColorVertex, point.coords[0])), 0, 0, 0, 0 },
	{ _strdup("y"), PLYType< Real >(), PLYType< Real >(), int(offsetof(PlyOrientedColorVertex, point.coords[1])), 0, 0, 0, 0 },
	{ _strdup("z"), PLYType< Real >(), PLYType< Real >(), int(offsetof(PlyOrientedColorVertex, point.coords[2])), 0, 0, 0, 0 },
	{ _strdup("nx"), PLYType< Real >(), PLYType< Real >(), int(offsetof(PlyOrientedColorVertex, normal.coords[0])), 0, 0, 0, 0 },
	{ _strdup("ny"), PLYType< Real >(), PLYType< Real >(), int(offsetof(PlyOrientedColorVertex, normal.coords[1])), 0, 0, 0, 0 },
	{ _strdup("nz"), PLYType< Real >(), PLYType< Real >(), int(offsetof(PlyOrientedColorVertex, normal.coords[2])), 0, 0, 0, 0 },
	{ _strdup("red"), PLYType< unsigned char >(), PLYType< unsigned char >(), int(offsetof(PlyOrientedColorVertex, color[0])), 0, 0, 0, 0 },
	{ _strdup("green"), PLYType< unsigned char >(), PLYType< unsigned char >(), int(offsetof(PlyOrientedColorVertex, color[1])), 0, 0, 0, 0 },
	{ _strdup("blue"), PLYType< unsigned char >(), PLYType< unsigned char >(), int(offsetof(PlyOrientedColorVertex, color[2])), 0, 0, 0, 0 },
	{ _strdup("r"), PLYType< unsigned char >(), PLYType< unsigned char >(), int(offsetof(PlyOrientedColorVertex, color[0])), 0, 0, 0, 0 },
	{ _strdup("g"), PLYType< unsigned char >(), PLYType< unsigned char >(), int(offsetof(PlyOrientedColorVertex, color[1])), 0, 0, 0, 0 },
	{ _strdup("b"), PLYType< unsigned char >(), PLYType< unsigned char >(), int(offsetof(PlyOrientedColorVertex, color[2])), 0, 0, 0, 0 }
};
template< class Real > PlyProperty PlyOrientedColorVertex< Real >::WriteProperties[] =
{
	{ _strdup("x"), PLYType< Real >(), PLYType< Real >(), int(offsetof(PlyOrientedColorVertex, point.coords[0])), 0, 0, 0, 0 },
	{ _strdup("y"), PLYType< Real >(), PLYType< Real >(), int(offsetof(PlyOrientedColorVertex, point.coords[1])), 0, 0, 0, 0 },
	{ _strdup("z"), PLYType< Real >(), PLYType< Real >(), int(offsetof(PlyOrientedColorVertex, point.coords[2])), 0, 0, 0, 0 },
...
template< class Real >
class PlyValueOrientedColorVertex
{
public:
	struct _PlyValueOrientedColorVertex
	{
		Point3D< Real > point, normal, color;
		Real value;
		_PlyValueOrientedColorVertex(void) { ; }
		_PlyValueOrientedColorVertex(Point3D< Real > p, Real v, Point3D< Real > n, Point3D< Real > c) : point(p), value(v), normal(n), color(c) { ; }
		_PlyValueOrientedColorVertex(PlyValueOrientedColorVertex< Real > p) { point = p.point; value = p.value; normal = p.normal; for (int c = 0; c < 3; c++) color[c] = (Real)p.color[c]; }
		operator PlyValueOrientedColorVertex< Real >()
		{
			PlyValueOrientedColorVertex< Real > p;
			p.point = point;
			p.normal = normal;
			p.value = value;
			for (int c = 0; c<3; c++) p.color[c] = (unsigned char)std::max< int >(0, std::min< int >(255, (int)(color[c] + 0.5)));
		...
		_PlyValueOrientedColorVertex operator + (_PlyValueOrientedColorVertex p) const { return _PlyValueOrientedColorVertex(point + p.point, normal + p.normal, color + p.color); }
		_PlyValueOrientedColorVertex operator - (_PlyValueOrientedColorVertex p) const { return _PlyValueOrientedColorVertex(point - p.point, normal - p.normal, color - p.color); }
		template< class _Real > _PlyValueOrientedColorVertex operator * (_Real s) const { return _PlyValueOrientedColorVertex(point*s, normal*s, color*s); }
		template< class _Real > _PlyValueOrientedColorVertex operator / (_Real s) const { return _PlyValueOrientedColorVertex(point / s, normal / s, color / s); }
		_PlyValueOrientedColorVertex& operator += (_PlyValueOrientedColorVertex p) { point += p.point, normal += p.normal, color += p.color; return *this; }
		_PlyValueOrientedColorVertex& operator -= (_PlyValueOrientedColorVertex p) { point -= p.point, normal -= p.normal, color -= p.color; return *this; }
		template< class _Real > _PlyValueOrientedColorVertex& operator *= (_Real s) { point *= s, normal *= s; color *= s; return *this; }
		template< class _Real > _PlyValueOrientedColorVertex& operator /= (_Real s) { point /= s, normal /= s; color /= s; return *this; }
	};

	typedef _PlyValueOrientedColorVertex Wrapper;

	const static int ReadComponents = 13;
	const static int WriteComponents = 10;
	static PlyProperty ReadProperties[];
	static PlyProperty WriteProperties[];
	...
	operator Point3D< Real >& () { return point; }
	operator const Point3D< Real >& () const { return point; }
	PlyValueOrientedColorVertex(void) { point.coords[0] = point.coords[1] = point.coords[2] = 0, normal.coords[0] = normal.coords[1] = normal.coords[2] = 0; color[0] = color[1] = color[2] = 0; }
	PlyValueOrientedColorVertex(const Point3D<Real>& p) { point = p; }
	PlyValueOrientedColorVertex(const Point3D< Real >& p, const unsigned char c[3]) { point = p, color[0] = c[0], color[1] = c[1], color[2] = c[2]; }
};
template< class Real, class _Real > PlyValueOrientedColorVertex< Real > operator * (XForm4x4< _Real > xForm, PlyValueOrientedColorVertex< Real > v) { return PlyValueOrientedColorVertex< Real >(xForm * v.point, xForm.inverse().transpose() * v.normal, v.color); }

template< class Real > PlyProperty PlyValueOrientedColorVertex< Real >::ReadProperties[] =
{
	{ _strdup("x"), PLYType< Real >(), PLYType< Real >(), int(offsetof(PlyValueOrientedColorVertex, point.coords[0])), 0, 0, 0, 0 },
	{ _strdup("y"), PLYType< Real >(), PLYType< Real >(), int(offsetof(PlyValueOrientedColorVertex, point.coords[1])), 0, 0, 0, 0 },
	{ _strdup("z"), PLYType< Real >(), PLYType< Real >(), int(offsetof(PlyValueOrientedColorVertex, point.coords[2])), 0, 0, 0, 0 },
	{ _strdup("value") , PLYType<          Real >() , PLYType<          Real >() , int(offsetof(PlyValueOrientedColorVertex ,        value)) , 0 , 0 , 0 , 0 } ,
	{ _strdup("nx"), PLYType< Real >(), PLYType< Real >(), int(offsetof(PlyValueOrientedColorVertex, normal.coords[0])), 0, 0, 0, 0 },
	{ _strdup("ny"), PLYType< Real >(), PLYType< Real >(), int(offsetof(PlyValueOrientedColorVertex, normal.coords[1])), 0, 0, 0, 0 },
	{ _strdup("nz"), PLYType< Real >(), PLYType< Real >(), int(offsetof(PlyValueOrientedColorVertex, normal.coords[2])), 0, 0, 0, 0 },
	{ _strdup("red"), PLYType< unsigned char >(), PLYType< unsigned char >(), int(offsetof(PlyValueOrientedColorVertex, color[0])), 0, 0, 0, 0 },
	{ _strdup("green"), PLYType< unsigned char >(), PLYType< unsigned char >(), int(offsetof(PlyValueOrientedColorVertex, color[1])), 0, 0, 0, 0 },
	{ _strdup("blue"), PLYType< unsigned char >(), PLYType< unsigned char >(), int(offsetof(PlyValueOrientedColorVertex, color[2])), 0, 0, 0, 0 },
	{ _strdup("r"), PLYType< unsigned char >(), PLYType< unsigned char >(), int(offsetof(PlyValueOrientedColorVertex, color[0])), 0, 0, 0, 0 },
	{ _strdup("g"), PLYType< unsigned char >(), PLYType< unsigned char >(), int(offsetof(PlyValueOrientedColorVertex, color[1])), 0, 0, 0, 0 },
	{ _strdup("b"), PLYType< unsigned char >(), PLYType< unsigned char >(), int(offsetof(PlyValueOrientedColorVertex, color[2])), 0, 0, 0, 0 }
};
template< class Real > PlyProperty PlyValueOrientedColorVertex< Real >::WriteProperties[] =
{
	{ _strdup("x"), PLYType< Real >(), PLYType< Real >(), int(offsetof(PlyValueOrientedColorVertex, point.coords[0])), 0, 0, 0, 0 },
	{ _strdup("y"), PLYType< Real >(), PLYType< Real >(), int(offsetof(PlyValueOrientedColorVertex, point.coords[1])), 0, 0, 0, 0 },
	{ _strdup("z"), PLYType< Real >(), PLYType< Real >(), int(offsetof(PlyValueOrientedColorVertex, point.coords[2])), 0, 0, 0, 0 },
